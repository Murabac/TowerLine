<x-app-layout>
    <div class="p-4 lg:p-8 max-w-3xl">
        ...
        <h1 class="mt-4 text-2xl font-semibold tracking-tight text-brand">{{ __('app.licenses.create') }}</h1>

        <form method="POST" action="{{ route('licenses.store') }}" enctype="multipart/form-data" class="mt-5 space-y-5">
            @csrf
            <section class="data-card p-5 sm:p-6 space-y-6">
                <div>
                    <label class="text-sm font-semibold text-gray-900">{{ __('app.licenses.tower') }}</label>
                    <select name="tower_id" class="field mt-2" required>
                        <option value="">{{ __('app.licenses.tower') }}</option>
                        @foreach ($towers as $tower)
                            <option value="{{ $tower->id }}" @selected((string) old('tower_id', request('tower_id')) === (string) $tower->id)>
                                {{ $tower->name }} · {{ $tower->operator->name }}
                            </option>
                        @endforeach
                    </select>
                    @error('tower_id') <p class="mt-1 text-sm text-red-600">{{ $message }}</p> @enderror
                </div>
