<x-app-layout>
    <div class="p-4 lg:p-8 max-w-4xl">
        ...
        <div class="mt-4 rounded-2xl bg-brand text-white px-5 py-5 sm:px-6">
            <p class="text-[11px] font-semibold uppercase tracking-[0.14em] text-brand-light">{{ __('app.users.title') }}</p>
            <h1 class="mt-1 text-2xl font-semibold tracking-tight">{{ __('app.users.edit') }}</h1>
            <p class="mt-2 text-sm text-white/75">{{ $managedUser->email }} · {{ $managedUser->roleLabel() }}</p>
        </div>

        <form method="POST" action="{{ route('users.update', $managedUser) }}" class="mt-5 space-y-5">
            @csrf
            @method('PUT')
